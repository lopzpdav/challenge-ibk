package com.retoibk.products.bcs.adapter.config.aop;
...
public class LoggingAspect {
    ...
    @Pointcut("execution(* com.retoibk.products.bcs.adapter.web..*(..))")
    ...
    public void adviceAfterSuccesfulRequest (JoinPoint jp, Object result){
        ...
        if (result instanceof Flux<?>) {
            ((Flux<?>) result).doOnNext(response -> log.info("Response: {}", response))
                    .subscribe();
        }
